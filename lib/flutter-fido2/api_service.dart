...
class ApiService {
  ...
  Future<String?> read({required String key}) async {
    try {
      ...
      if (baseUrl == 'http://fido.local') {
        return await _storage.read(key: key);
      }

      final response = await http
          .get(Uri.parse('$baseUrl/read.php?key=${Uri.encodeComponent(key)}'));
  ...
  Future<String?> write({required String key, required String value}) async {
    try {
      ...
      print(
          'url is ${Uri.parse('$baseUrl/write.php?key=${Uri.encodeComponent(key)}&d=${Uri.encodeComponent(value)}')}');
      final response = await http.get(Uri.parse(
          '$baseUrl/write.php?key=${Uri.encodeComponent(key)}&d=${Uri.encodeComponent(value)}'));
      if (response.statusCode != 200) return null;
      final data = response.body;
      final String result = data;
      return result;
  ...
  Future<String?> delete({required String key}) async {
    try {
      ...
      final response = await http
          .get(Uri.parse('$baseUrl/del.php?key=${Uri.encodeComponent(key)}'));
      if (response.statusCode != 200) return null;
      final data = response.body;
      final String result = data;
      return result;
